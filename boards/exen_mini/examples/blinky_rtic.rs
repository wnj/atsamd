...
mod app {
    ...
    fn init(cx: init::Context) -> (Shared, Local, init::Monotonics) {
        ...
        let blue_led: bsp::BlueLed = pins.a8.into();
        ...
        (Shared { blue_led }, Local {}, init::Monotonics(rtc))
    }

    #[task(shared = [blue_led])]
    fn blink(mut cx: blink::Context) {
        // If the LED were a local resource, the lock would not be necessary
        cx.shared.blue_led.lock(|led| led.toggle().unwrap());
