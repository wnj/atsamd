...
fn main() -> ! {
    ...
    loop {
        ...
        // let mut x = String::<U16>::new();
        // x = "hallo\r\n";
        // write_serial(x.as_bytes());

        let time = disable_interrupts(|_| unsafe { RTC.as_mut().map(|rtc| rtc.current_time()) }).unwrap();
